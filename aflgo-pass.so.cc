...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  // ! is_aflgo_preprocessing determines if the source code is compiled for the FIRST TIME or not
  // ! According to the AFLGO paper, we need compile the source code twice.
  ...
  if (is_aflgo_preprocessing) {
    ...
    for (auto &F : M) {
      ...
      // * After running this method, we can obtain the BB name and is_target is set to determine if this BB contains target
      // * filename:line or not.
      for (auto &BB : F) {
        ...
        // * After exec, the bb_name will have a value (which is determined by the location [represent as filename:line]
        // * of the first instruction)
        // * When entering the for loop for the first time, bb_name will be set.
        for (auto &I : BB) {

            // * obtain the basic info about current instruction
            // * filename:    The location of I
            // * line:        The line of I
            getDebugLoc(&I, filename, line);

            // * External lib starts with /usr/
            // * Like we will not consider: /usr/lib/XXX.c:xxx
            static const std::string Xlibs("/usr/");
            if (filename.empty() || line == 0 || !filename.compare(0, Xlibs.size(), Xlibs))
                continue;

            // * handle the filename into correct corresponding file
            std::size_t found = filename.find_last_of("/\\");
            if (found != std::string::npos)
                filename = filename.substr(found + 1);

            // * bb_name initialization
            // * Using the first valid (not in the black list) instruction in BasicBlock to be used as bb_name
            if (bb_name.empty())
                bb_name = filename + ":" + std::to_string(line);

            // * Now we have prepared information for the instruction
            // * like:
            // * I: main.c:1145
            // * To find if instruction is the target
            ...
            if (auto *c = dyn_cast<CallInst>(&I)) {
                ...
                // * If so, send the information to bbcalls
        ...
        // ! Right after for (auto &I : BB) HERE

        // * bb_name is obtained by the Iteration of for(auto &I : BB). It may be empty. If it's not, then
        // * use bb_name for BB_name. While BasicBlocks may be with no name (as indicated by hasName() method)
        // * We will use allocator to avoid name conflict.
        // ! We should know that this could happen if the code in the source file contains
        // ! many basic block in a single line.
        if (!bb_name.empty()) {
            BB.setName(bb_name + ":");

            // * If BB.hasName() == 0, means that we have name conflicts. Use another name
            if (!BB.hasName()) {
      ...
      // ! Right after for (auto &BB : F) HERE
      // * has_BBs
      if (has_BBs) {
        /* Print CFG */
